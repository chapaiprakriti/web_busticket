...
- Name: profile.spec.ts >> Profile Page >> ai chat widget button is visible on profile
- Location: tests\profile.spec.ts:20:7
...
Locator: locator('button[aria-label*="chat" i], button[aria-label*="ai" i]').first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('button[aria-label*="chat" i], button[aria-label*="ai" i]').first()
